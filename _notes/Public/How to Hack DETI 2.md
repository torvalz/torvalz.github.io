...
let completeSession = (player = window.player) => { return new Promise(async(resolve, reject) => {
	...
	let outerLoop = setInterval(async() => {
		...
		if (player._itemMap.get('lrnStatus') == 'N') {
			// 시간 조작
			// 진도 완료
			console.log('시간 및 진도 조작 시작')
			player._itemMap.put('lrnStatus', 'Y')
			player._itemMap.put('lrnComptYn', 'Y')
			player._itemMap.put('befConectTms', 700 + parseInt(500*Math.random()))
			await player.$.ajax({
					url : '/lcms/nonscorm/initProgress.go',
					cache : false,
					async : false,
					type : 'POST',
					timeout : 1000,
					data : player._itemMap.toString(),
					dataType : "json",
					error : function(data) {
						alert('error : ' + data);
						result = false;
